  ...
  if (isFeatureEnabled('KYC_ENABLED')) {
    ...
    if (profile.kyc_status !== 'approved') {
      ...
      if (profile.kyc_status === 'pending') {
        ...
        errorDetails =
          "Votre vérification d'identité est en cours d'examen. Vous pourrez déposer des colis une fois celle-ci approuvée. Délai indicatif : 24 à 48 h."
  ...
  if (isFeatureEnabled('KYC_ENABLED')) {
    ...
    if (profile.kyc_status !== 'approved') {
      ...
      if (profile.kyc_status === 'pending') {
        errorMessage = 'Vérification en cours'
        errorDetails =
          "Votre vérification d'identité est en cours d'examen. Vous pourrez livrer des colis une fois celle-ci approuvée. Délai indicatif : 24 à 48 h."
      } else if (profile.kyc_status === 'rejected') {
        errorMessage = 'Vérification refusée'
        errorDetails = profile.kyc_rejection_reason
          ? `Votre vérification a été refusée : ${profile.kyc_rejection_reason}. Veuillez soumettre de nouveaux documents.`
  ...
  if (isFeatureEnabled('KYC_ENABLED')) {
    ...
    if (profile.kyc_status !== 'approved') {
      ...
      if (profile.kyc_status === 'pending') {
        errorMessage = 'Vérification en cours'
        errorDetails =
          "Votre vérification d'identité est en cours d'examen. Vous pourrez confirmer la réception une fois celle-ci approuvée. Délai indicatif : 24 à 48 h."
      } else if (profile.kyc_status === 'rejected') {
        errorMessage = 'Vérification refusée'
        errorDetails = profile.kyc_rejection_reason
          ? `Votre vérification a été refusée : ${profile.kyc_rejection_reason}. Veuillez soumettre de nouveaux documents.`
