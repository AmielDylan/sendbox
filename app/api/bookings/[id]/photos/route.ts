...
export async function POST(
  ...
  if (!['confirmed', 'handed', 'in_transit'].includes(booking.status)) {
    return NextResponse.json({ error: 'Statut incompatible pour upload photo', code: 'INVALID_STATUS' }, { status: 422 })
  }

  let body: { url: string; type: 'handoff' | 'delivery'; sizeBytes: number; uploadSecret?: string }
  try {
    body = await req.json()
  } catch {
  ...
  if (UPLOAD_SECRET && body.uploadSecret !== UPLOAD_SECRET) {
    return NextResponse.json({ error: 'Secret upload invalide', code: 'INVALID_SECRET' }, { status: 403 })
  }

  if (!body.url || !body.type || !['handoff', 'delivery'].includes(body.type)) {
    return NextResponse.json({ error: 'Paramètres manquants ou invalides', code: 'BAD_REQUEST' }, { status: 400 })
  }

  if (body.sizeBytes > MAX_PHOTO_SIZE_BYTES) {
    return NextResponse.json({ error: 'Fichier trop volumineux', code: 'FILE_TOO_LARGE' }, { status: 413 })
  }

  // capturedAt est TOUJOURS généré serveur — jamais extrait des métadonnées EXIF
  const capturedAt = new Date().toISOString()
  const fileHash = createHash('sha256').update(body.url + capturedAt).digest('hex')
  ...
  if (error) {
    ...
    return NextResponse.json({ error: "Erreur lors de l'enregistrement de la photo", code: 'DB_ERROR' }, { status: 500 })
  }
