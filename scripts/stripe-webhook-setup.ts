...
function startStripeListen() {
  ...
  stripeProcess.stdout.on('data', (data: Buffer) => {
    ...
    if (!secretExtracted) {
      ...
      if (match) {
        ...
        console.log('💡 Le secret webhook a été automatiquement ajouté à .env.local')
        console.log('💡 Vous pouvez maintenant redémarrer votre serveur Next.js si nécessaire\n')
      }
    }
  })
  ...
  stripeProcess.on('error', (error) => {
    console.error('❌ Erreur lors du lancement de stripe listen:', error.message)
