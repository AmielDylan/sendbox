...
export function createProfile(profile: ProfileBase): Profile {
  ...
  return {
    ...
    display_name: fullName || profile.id.slice(0, 8),
  }
}
...
export function createAnnouncement(
  ...
  const remainingKg = Math.max(0, announcement.available_kg - totalBookedKg)
...
export function createNotification(
  ...
  // Générer action_url depuis booking_id ou announcement_id
  let actionUrl: string | undefined
  if (notification.booking_id) {
    actionUrl = `/dashboard/bookings/${notification.booking_id}`
  } else if (notification.announcement_id) {
    actionUrl = `/dashboard/annonces/${notification.announcement_id}`
  }
