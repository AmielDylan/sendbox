-- Migration: Fix ambiguous status column and EXTRACT casting issues
-- Created: 2026-01-19
-- Description: Prefix status with table alias and fix date interval casting

-- Drop and recreate search_announcements with prefixed status column
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      (SELECT COUNT(*)::BIGINT
       FROM bookings b
       WHERE b.announcement_id = a.id AND b.status = 'completed'),
      0
...
-- Drop and recreate count_search_announcements with AGE function
...
-- Add comments
COMMENT ON FUNCTION search_announcements IS 'Public search for announcements. No authentication required. Uses AGE() for date calculations.';
COMMENT ON FUNCTION count_search_announcements IS 'Count announcements matching search filters. No authentication required. Uses AGE() for date calculations.';
