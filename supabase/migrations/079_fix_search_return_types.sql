-- Migration: Fix search_announcements return types to match actual table columns
-- Created: 2026-01-19
-- Description: Change departure_date and arrival_date return types from TIMESTAMPTZ to DATE

-- Drop and recreate search_announcements with correct return types
    ...
    (
      ...
      CASE
        WHEN p_departure_date IS NOT NULL THEN
          CASE
            WHEN a.departure_date = p_departure_date THEN 20
            WHEN ABS(EXTRACT(DAY FROM AGE(a.departure_date, p_departure_date))) <= 1 THEN 15
            WHEN ABS(EXTRACT(DAY FROM AGE(a.departure_date, p_departure_date))) <= 2 THEN 10
            WHEN ABS(EXTRACT(DAY FROM AGE(a.departure_date, p_departure_date))) <= 3 THEN 5
            ELSE 0
          END
        ELSE 0
      END +
...
-- Add comment
COMMENT ON FUNCTION search_announcements IS 'Public search for announcements. Returns DATE types for departure_date and arrival_date.';
